...
**批注样式复刻旧版**：全部标记工具（高亮/下划线/删除线/框选/画笔/自由高亮/文本框）换回旧版 lime 颜色；mark 行内合并容差加大（修复部分行断段）。

**批注↔卡片选中模型**：双向选中 2s 自动消失（卡片高亮 + PDF 环一起）；空点击立即取消；点卡片/批注同时高亮、一起消失。

**无项目捕获**：移除新建项目弹窗——无项目时保存失败 + 面板提示「请先创建一个项目」+ 草稿内容保留 + 保存按钮禁用；新增**网页右下角 Lime 悬浮球**（菜单「打开面板」唤起捕获侧栏，替代 Alt+L 快捷键）。
