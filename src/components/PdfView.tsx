...
export default function PdfView({
  ...
  // The engine bytes + annotations for the loaded PDF.
  useEffect(() => {
    let cancelled = false
    setStores([])
    setBytes(null)
  ...
  const handleAdd = useCallback(
    async (annotation: IAnnotationStore, pos?: { x: number; y: number }) => {
      if (!loaded) return
      try {
        await saveAnnotationFromStore({ pdfId: loaded.file.id, store: annotation, pos })
      } catch (e) {
