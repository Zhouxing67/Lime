...
function generateUUID(): string {
    // nanoid() crashed in the bundled extension context (crypto.getRandomValues
    // returned undefined on some paths) — prefer crypto.randomUUID() and fall
    // back to a fully self-contained generator.
    try {
...
function convertToRGB(array: number[], index = 0) {
...
function formatPDFDate(dateString: string | null, full = false): string {
    ...
    if (full) {
        return i18n.t('common:dateFormat.full', { year, month, day, hour, minute })
    }
    ...
    if (year === currentYear) {
        return i18n.t('common:dateFormat.dayMonth', { day, month })
    }

    return i18n.t('common:dateFormat.dayMonthYear', { day, month, year })
}
