...
export class EditorFreeHighlight extends Editor {
    ...
    private correctLineIfStraight(points: number[]): number[] {
        ...
        // 如果 deltaX 和 deltaY 都为 0，则是点而不是线
        if (deltaX === 0 && deltaY === 0) {
            return points
        }
        ...
        // 判断线段是否接近水平或垂直
        // 水平线包括接近0度和接近180度的情况
        const isCloseToHorizontal =
            angleDeg <= THRESHOLD_ANGLE_DEGREES ||
            angleDeg >= 180 - THRESHOLD_ANGLE_DEGREES ||
            (angleDeg >= 90 - THRESHOLD_ANGLE_DEGREES && angleDeg <= 90 + THRESHOLD_ANGLE_DEGREES && Math.abs(deltaX) > Math.abs(deltaY))

        // 垂直线包括接近90度的情况
        const isCloseToVertical =
            (angleDeg >= 90 - THRESHOLD_ANGLE_DEGREES && angleDeg <= 90 + THRESHOLD_ANGLE_DEGREES && Math.abs(deltaY) > Math.abs(deltaX)) ||
            angleDeg >= 180 - THRESHOLD_ANGLE_DEGREES ||
            angleDeg <= THRESHOLD_ANGLE_DEGREES

        if (isCloseToHorizontal) {
            // 修正为水平线，所有点的Y坐标都等于起始点的Y坐标
            return points.map((value, index) => (index % 2 === 0 ? value : startY))
        } else if (isCloseToVertical) {
            // 修正为垂直线，所有点的X坐标都等于起始点的X坐标
            return points.map((value, index) => (index % 2 === 0 ? startX : value))
        }
