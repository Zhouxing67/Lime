...
export abstract class Editor {
    ...
    protected defaultOptions: PdfAnnotatorOptions
    ...
    protected delShapeGroup(id: string) {
        ...
        const group = this.konvaStage.findOne((node: Konva.Shape) => node.getType() === 'Group' && node.id() === id) // 查找对应 ID 的 Konva.Group 对象
        if (group) {
